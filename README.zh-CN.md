<div align="center">
  ...
  <p>
    ...
    ·
    <a href="docs/assets/stability-report/visual-parity-evidence.md"><strong>2,304 案例视觉报告</strong></a>
    ·
...
> Stable。** 当前支持的 9 类图均已通过独立生产场景语料、
> 2,304 案例 Native/Official 视觉矩阵、确定性重放、独立随机压力测试、
> 运行时负载测试和跨平台构建。
>
> 建议先查看 **[2,304 案例 Native/Official 视觉报告](docs/assets/stability-report/visual-parity-evidence.md)**，
> 其中包含 4,608 张截图和 144 页分页对比图。
...
| 独立生产场景 | 119 |
| 已声明能力覆盖率 | 144/144 |
| 大规模视觉矩阵 | 2,304 个唯一源码，每类图 256 个 |
| Native/Official 截图 | 4,608 张矩阵截图，另有 238 张独立语料截图 |
| 人工视觉审查 | 119 个通过，0 个阻塞 |
| 自动视觉几何检查 | 2,304/2,304 个矩阵对和 119/119 个独立对通过 |
| 确定性 SceneGraph 重放 | 119 个通过，0 个不一致 |
| 内置主题矩阵 | 99/99 |
| 独立生成的 Native 压力输入 | 2,304 |
| JVM 测试 | 296 个通过，0 个失败 |
| Core 生产场景压力测试 | 595 次渲染，P95 为 76ms |
| 运行时负载矩阵 | Android、iOS、Desktop、Web 全部通过 |

| 证据文档 | 内容 |
| --- | --- |
| **[Stable 测试报告](docs/stability-report.md)** | Stable 判定、视觉对比图、测试、压力指标、运行时负载证据和复现步骤 |
| **[全部 2,304 个 Native/Official 对比](docs/assets/stability-report/visual-parity-evidence.md)** | 144 页分页对比图，每页包含 16 组同源码结果 |
| [生产能力矩阵](docs/production-capability-matrix.md) | 144 项被独立验证的能力 |
...
| [Full Visual Parity](https://github.com/swithun-liu/cmp-mermaid/actions/workflows/full-visual-parity.yml) | 每周或手动执行的 9 路矩阵截图、几何门禁和可下载证据 |
...
测试报告包含 119 个独立生产场景对比，以及由 2,304 个唯一 Mermaid 源码组成的
独立大规模矩阵：
**[查看全部 144 页视觉证据](docs/assets/stability-report/visual-parity-evidence.md)**。
...
在各自文档声明的兼容范围内，9 类图均支持 Mermaid frontmatter、metadata、
Unicode 和相关主题变量。
