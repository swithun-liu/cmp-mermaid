...
const nativeDirectory = resolve(
  repositoryRoot,
  process.env.NATIVE_DIR ?? 'captures/local/xychart-native',
);
const officialDirectory = resolve(
  repositoryRoot,
  process.env.OFFICIAL_DIR ?? 'captures/local/xychart-official',
);
const outputDirectory = resolve(
  repositoryRoot,
  process.env.OUTPUT_DIR ?? 'captures/local/xychart-contact-sheets',
);
...
if (caseIds.length === 0) {
  throw new Error('No matching Native/Official XY Chart screenshots were found');
}
...
try {
  for (let start = 0; start < caseIds.length; start += casesPerSheet) {
    ...
    await page.screenshot({
      path: resolve(outputDirectory, `xychart-contact-${sheetNumber}.png`),
      fullPage: true,
...
console.log(`Rendered ${caseIds.length} XY Chart comparison pairs to ${outputDirectory}`);
