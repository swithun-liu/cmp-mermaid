...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^agentflow-beta(?::|\s|$)/.test(candidate));
    if (/^(mermaid|mermaid-example)$/.test(fenceLanguage) && hasDiagramHeader) {
      cases.push({
        id: `${String(cases.length + 1).padStart(3, '0')}_${slug(heading)}`,
        title: heading,
        source,
        expectedUnsupportedFeature: hasExplicitElkLayout(source) ? 'ELK layout' : null,
      });
    }
    fenceLanguage = '';
    fenceLines = null;
...
const kotlinCases = cases
  .map(
    ({ id, title, source, expectedUnsupportedFeature }) => `    MermaidAgentflowDocCase(
        id = ${kotlinString(id)},
        title = ${kotlinString(title)},
        source = ${kotlinRawString(source)},
        expectedUnsupportedFeature = ${
          expectedUnsupportedFeature === null
            ? 'null'
            : kotlinString(expectedUnsupportedFeature)
        },
    )`,
  )
  .join(',\n');
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialAgentflowDocumentationCases.kt',
);
...
console.log(
  `Generated ${cases.length} Mermaid ${MERMAID_VERSION} Agentflow documentation fixtures.`,
);

function hasExplicitElkLayout(source) {
  return /^\s*layout:\s*["']?elk(?:["']?\s*)$/m.test(source);
}
