...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^usecase-beta(?:\s|$)/.test(candidate));
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidUsecaseDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  ...
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/' +
    'OfficialUsecaseDocumentationCases.kt',
);
...
console.log(`Generated ${cases.length} Mermaid ${MERMAID_VERSION} Use Case fixtures.`);
