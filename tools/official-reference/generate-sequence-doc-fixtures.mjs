...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^sequenceDiagram\b/.test(candidate));
    if (
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidSequenceDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialSequenceDocumentationCases.kt',
);
...
/**
 * Generated from Mermaid ${MERMAID_VERSION} packages/mermaid/src/docs/syntax/sequenceDiagram.md.
...
console.log(
  `Generated ${cases.length} Mermaid ${MERMAID_VERSION} Sequence documentation fixtures.`,
);
...
function kotlinRawString(value) {
  if (value.includes('"""')) {
    throw new Error('Sequence fixture contains a Kotlin raw-string delimiter');
  }
  return `"""
${value.replaceAll('$', "${'$'}")}
        """.trimIndent()`;
