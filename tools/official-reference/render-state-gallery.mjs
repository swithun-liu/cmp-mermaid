...
const sourcePath = resolve(
  repositoryRoot,
  'mermaid-debug-ui/src/commonMain/kotlin/com/swithun/cmpmermaid/debugui/StateDemos.kt',
);
const output = resolve(
  repositoryRoot,
  process.env.OUTPUT_DIR ?? 'captures/local/state-official',
);
...
const layout = process.env.STATE_LAYOUT ?? 'dagre';
...
const casePattern =
  /StateDemo\(\s*id = "([^"]+)",\s*title = "([^"]+)",\s*category = "([^"]+)",\s*source = """\n([\s\S]*?)\n\s*"""\.trimIndent\(\),\s*\)/g;
...
if (cases.length === 0) {
  throw new Error('No State gallery cases matched StateDemos.kt');
}
  ...
  for (const demo of cases) {
    await page.evaluate(
      async ({ id, source, layout }) => {
        ...
        const { svg } = await mermaid.render(`official-state-${id}`, source, container);
...
console.log(`Rendered ${cases.length} State gallery references to ${output}`);
