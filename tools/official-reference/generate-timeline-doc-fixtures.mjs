...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^timeline(?:\s+(?:LR|TD))?\s*$/i.test(candidate));
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidTimelineDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialTimelineDocumentationCases.kt',
);
...
console.log(
  `Generated ${cases.length} Mermaid ${MERMAID_VERSION} Timeline documentation fixtures.`,
);
...
function kotlinRawString(value) {
  if (value.includes('"""')) {
    throw new Error('Timeline fixture contains a Kotlin raw-string delimiter');
  }
  return `"""
${value.replaceAll('$', () => "${'$'}")}
        """.trimIndent()`;
