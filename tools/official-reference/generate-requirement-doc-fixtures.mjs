...
const documentationHash = crypto
  ...
  .digest('hex');
const cases = [];
let heading = 'Requirement diagrams';
let fenceLanguage = '';
let fenceLines = null;
...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^requirementDiagram\b/i.test(candidate));
    if (
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidRequirementDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialRequirementDocumentationCases.kt',
);
...
console.log(
  `Generated ${cases.length} Mermaid ${MERMAID_VERSION} Requirement documentation fixtures.`,
);
...
function kotlinRawString(value) {
  if (value.includes('"""')) {
    throw new Error('Requirement fixture contains a Kotlin raw-string delimiter');
  }
  return `\"\"\"\n${value}\n        \"\"\".trimIndent()`;
}
