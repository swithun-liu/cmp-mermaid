...
const nativeDirectory = resolve(
  repositoryRoot,
  process.env.NATIVE_DIR ?? 'captures/local/state-native',
);
const officialDirectory = resolve(
  repositoryRoot,
  process.env.OFFICIAL_DIR ?? 'captures/local/state-official',
);
const outputDirectory = resolve(
  repositoryRoot,
  process.env.OUTPUT_DIR ?? 'captures/local/state-contact-sheets',
);
...
if (caseIds.length === 0) {
  throw new Error('No matching Native/Official State screenshots were found');
}
...
try {
  for (let start = 0; start < caseIds.length; start += casesPerSheet) {
    ...
    await page.screenshot({
      path: resolve(outputDirectory, `state-contact-${sheetNumber}.png`),
      fullPage: true,
...
console.log(
  `Rendered ${caseIds.length} State comparison pairs to ${outputDirectory}`,
);
