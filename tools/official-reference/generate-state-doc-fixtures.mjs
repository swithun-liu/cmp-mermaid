...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^stateDiagram(?:-v2)?\b/.test(candidate));
    if (
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidStateDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialStateDocumentationCases.kt',
);
...
/**
 * Generated from Mermaid ${MERMAID_VERSION} packages/mermaid/src/docs/syntax/stateDiagram.md.
...
console.log(
  `Generated ${cases.length} Mermaid ${MERMAID_VERSION} State documentation fixtures.`,
);
...
function kotlinRawString(value) {
  if (value.includes('"""')) {
    throw new Error('State fixture contains a Kotlin raw-string delimiter');
  }
  return `"""
${value.replaceAll('$', "${'$'}")}
        """.trimIndent()`;
