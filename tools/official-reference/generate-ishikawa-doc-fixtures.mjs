...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^ishikawa(?:-beta)?\s*$/.test(candidate));
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidIshikawaDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialIshikawaDocumentationCases.kt',
);
...
console.log(
  `Generated ${cases.length} Mermaid ${MERMAID_VERSION} Ishikawa documentation fixtures.`,
);
...
function kotlinRawString(value) {
  if (value.includes('"""')) {
    throw new Error('Ishikawa fixture contains a Kotlin raw-string delimiter');
  }
  return `"""
${value.replaceAll('$', () => "${'$'}")}
        """.trimIndent()`;
