...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^quadrantChart\b/i.test(candidate));
    if (
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidQuadrantDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialQuadrantDocumentationCases.kt',
);
...
console.log(
  `Generated ${cases.length} Mermaid ${MERMAID_VERSION} Quadrant documentation fixtures.`,
);
...
function kotlinRawString(value) {
  if (value.includes('"""')) {
    throw new Error('Quadrant fixture contains a Kotlin raw-string delimiter');
  }
  return `"""
${value.replaceAll('$', () => "${'$'}")}
        """.trimIndent()`;
