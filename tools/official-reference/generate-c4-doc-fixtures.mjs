...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^C4(?:Context|Container|Component|Dynamic|Deployment)(?:\s|$)/.test(candidate));
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidC4DocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialC4DocumentationCases.kt',
);
...
console.log(`Generated ${cases.length} Mermaid ${MERMAID_VERSION} C4 fixtures.`);
