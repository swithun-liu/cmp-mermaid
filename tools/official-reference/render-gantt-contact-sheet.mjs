...
const nativeDirectory = resolve(
  repositoryRoot,
  process.env.NATIVE_DIR ?? 'captures/local/gantt-native',
);
const officialDirectory = resolve(
  repositoryRoot,
  process.env.OFFICIAL_DIR ?? 'captures/local/gantt-official',
);
const outputDirectory = resolve(
  repositoryRoot,
  process.env.OUTPUT_DIR ?? 'captures/local/gantt-contact-sheets',
);
...
if (caseIds.length === 0) {
  throw new Error('No matching Native/Official Gantt screenshots were found');
}
...
try {
  for (let start = 0; start < caseIds.length; start += casesPerSheet) {
    ...
    await page.screenshot({
      path: resolve(outputDirectory, `gantt-contact-${sheetNumber}.png`),
      fullPage: true,
...
console.log(`Rendered ${caseIds.length} Gantt comparison pairs to ${outputDirectory}`);
