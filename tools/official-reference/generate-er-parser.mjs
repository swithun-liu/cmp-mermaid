...
const ER_JISON_SHA256 =
  '4f7c96b05d15bcc9c0e753bb106a5b8a67cfa4c8b2e13faab1d7098b92f9e5be';
const ER_MARKER =
...
if (sourceRoot) {
  ...
  const grammarHash = crypto
    ...
    .digest('hex');
  if (grammarHash !== ER_JISON_SHA256) {
    throw new Error(
      `Expected ER grammar ${ER_JISON_SHA256}, found ${grammarHash}`,
    );
  }
}
...
const erEntry = fs
  ...
  .find((name) => /^erDiagram-.*\.mjs$/.test(name));

if (!erEntry) {
  throw new Error('Unable to locate the Mermaid ER distribution entry');
}

const erChunk = fs.readFileSync(path.join(chunkDirectory, erEntry), 'utf8');
const parserStart = erChunk.indexOf(ER_MARKER) + ER_MARKER.length;
if (parserStart < ER_MARKER.length) {
  throw new Error('Unable to locate the generated Mermaid ER Jison parser');
}
const parserEnd = erChunk.indexOf(ER_END_MARKER, parserStart);
if (parserEnd < 0) {
  throw new Error('Unable to find the end of the generated ER parser');
}

const parserSource = `${erChunk.slice(parserStart, parserEnd)}\nparser`;
...
const cellSource = (cell) => {
  ...
  switch (cell[0]) {
    case 1:
      return `ErJisonCell.Shift(${cell[1]})`;
    case 2:
      return `ErJisonCell.Reduce(${cell[1]})`;
    case 3:
      return 'ErJisonCell.Accept';
    default:
...
lines.push(` * Upstream erDiagram.jison SHA-256: ${ER_JISON_SHA256}`);
lines.push(
  ` * Distribution chunk SHA-256: ${crypto.createHash('sha256').update(erChunk).digest('hex')}`,
);
lines.push(' *');
lines.push(' * Do not edit manually. Run:');
lines.push(' *   cd tools/official-reference && npm run generate:er-parser');
lines.push(' */');
lines.push('internal object ErJisonTables {');
...
lines.push('    )');
lines.push('');
lines.push('    val productions: Array<ErJisonProduction> = arrayOf(');
for (const production of parser.productions_) {
  if (production === 0) {
    lines.push('        ErJisonProduction(symbol = 0, length = 0),');
  } else {
    lines.push(
      `        ErJisonProduction(symbol = ${production[0]}, length = ${production[1]}),`,
    );
  }
}
lines.push('    )');
...
lines.push('    val states: Array<Map<Int, ErJisonCell>> = buildList {');
...
for (let chunkIndex = 0; chunkIndex < stateChunks.length; chunkIndex += 1) {
  lines.push(
    `    private fun stateChunk${chunkIndex}(): Array<Map<Int, ErJisonCell>> = arrayOf(`,
  );
...
lines.push('    val lexerPatterns: List<ErJisonLexerPattern> = buildList {');
...
for (let chunkIndex = 0; chunkIndex < lexerChunks.length; chunkIndex += 1) {
  lines.push(
    `    private fun lexerChunk${chunkIndex}(): List<ErJisonLexerPattern> = listOf(`,
  );
  for (
    let ruleIndex = 0;
    ...
    lines.push(
      `        ErJisonLexerPattern(${absoluteIndex}, ${kotlinString(rule.source)}),`,
    );
  }
  lines.push('    )');
  lines.push('');
