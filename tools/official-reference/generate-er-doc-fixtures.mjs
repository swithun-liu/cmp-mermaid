...
const documentationHash = crypto
  ...
  .digest('hex');
const cases = [];
let heading = 'Entity Relationship Diagrams';
let fenceLanguage = '';
let fenceLines = null;
...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^erDiagram\b/.test(candidate));
    if (
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidErDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialErDocumentationCases.kt',
);
...
console.log(
  `Generated ${cases.length} Mermaid ${MERMAID_VERSION} ER documentation fixtures.`,
);
...
function kotlinRawString(value) {
  if (value.includes('"""')) {
    throw new Error('ER fixture contains a Kotlin raw-string delimiter');
  }
  return `"""
${value.replaceAll('$', "${'$'}")}
        """.trimIndent()`;
