...
const nativeDirectory = resolve(
  repositoryRoot,
  process.env.NATIVE_DIR ?? 'captures/local/er-native',
);
const officialDirectory = resolve(
  repositoryRoot,
  process.env.OFFICIAL_DIR ?? 'captures/local/er-official',
);
const outputDirectory = resolve(
  repositoryRoot,
  process.env.OUTPUT_DIR ?? 'captures/local/er-contact-sheets',
);
...
if (caseIds.length === 0) {
  throw new Error('No matching Native/Official ER screenshots were found');
}
...
try {
  for (let start = 0; start < caseIds.length; start += casesPerSheet) {
    ...
    await page.screenshot({
      path: resolve(outputDirectory, `er-contact-${sheetNumber}.png`),
      fullPage: true,
...
console.log(`Rendered ${caseIds.length} ER comparison pairs to ${outputDirectory}`);
