...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^kanban\s*$/i.test(candidate));
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidKanbanDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialKanbanDocumentationCases.kt',
);
...
console.log(
  `Generated ${cases.length} Mermaid ${MERMAID_VERSION} Kanban documentation fixtures.`,
);
...
function kotlinRawString(value) {
  if (value.includes('"""')) {
    throw new Error('Kanban fixture contains a Kotlin raw-string delimiter');
  }
  return `"""
${value.replaceAll('$', () => "${'$'}")}
        """.trimIndent()`;
