...
const REQUIREMENT_JISON_SHA256 =
  '32030ae34660128966d31ecc25e5ce224ce9bf947e1e31bd3961f3879ae26ae7';
const REQUIREMENT_MARKER =
  '// src/diagrams/requirement/parser/requirementDiagram.jison\n';
const REQUIREMENT_END_MARKER = '\nparser.parser = parser;';
...
if (sourceRoot) {
  ...
  const grammarHash = crypto
    ...
    .digest('hex');
  if (grammarHash !== REQUIREMENT_JISON_SHA256) {
    throw new Error(
      `Expected Requirement grammar ${REQUIREMENT_JISON_SHA256}, found ${grammarHash}`,
    );
  }
}

const chunkDirectory = path.join(mermaidRoot, 'dist/chunks/mermaid.core');
const requirementEntry = fs
  .readdirSync(chunkDirectory)
  .find((name) => /^requirementDiagram-.*\.mjs$/.test(name));

if (!requirementEntry) {
  throw new Error('Unable to locate the Mermaid Requirement distribution entry');
}

const requirementChunk = fs.readFileSync(
  path.join(chunkDirectory, requirementEntry),
  'utf8',
);
const parserStart =
  requirementChunk.indexOf(REQUIREMENT_MARKER) + REQUIREMENT_MARKER.length;
if (parserStart < REQUIREMENT_MARKER.length) {
  throw new Error('Unable to locate the generated Mermaid Requirement Jison parser');
}
const parserEnd = requirementChunk.indexOf(REQUIREMENT_END_MARKER, parserStart);
if (parserEnd < 0) {
  throw new Error('Unable to find the end of the generated Requirement parser');
}

const parserSource = `${requirementChunk.slice(parserStart, parserEnd)}\nparser`;
...
const cellSource = (cell) => {
  if (typeof cell === 'number') {
    return `RequirementJisonCell.Goto(${cell})`;
  }
  ...
  switch (cell[0]) {
    case 1:
      return `RequirementJisonCell.Shift(${cell[1]})`;
    case 2:
      return `RequirementJisonCell.Reduce(${cell[1]})`;
    case 3:
      return 'RequirementJisonCell.Accept';
    default:
...
const lines = [];
lines.push('package com.swithun.cmpmermaid.core.requirement.upstream.mermaid');
lines.push('');
lines.push('/**');
lines.push(` * Generated from Mermaid ${MERMAID_VERSION} requirementDiagram.jison.`);
lines.push(` * Upstream requirementDiagram.jison SHA-256: ${REQUIREMENT_JISON_SHA256}`);
lines.push(
  ` * Distribution chunk SHA-256: ${crypto.createHash('sha256').update(requirementChunk).digest('hex')}`,
);
lines.push(' *');
lines.push(' * Do not edit manually. Run:');
lines.push(' *   cd tools/official-reference && npm run generate:requirement-parser');
lines.push(' */');
lines.push('internal object RequirementJisonTables {');
...
lines.push('    )');
lines.push('');
lines.push('    val productions: Array<RequirementJisonProduction> = arrayOf(');
for (const production of parser.productions_) {
  if (production === 0) {
    lines.push('        RequirementJisonProduction(symbol = 0, length = 0),');
  } else {
    lines.push(
      `        RequirementJisonProduction(symbol = ${production[0]}, length = ${production[1]}),`,
    );
  }
}
lines.push('    )');
...
lines.push('    val states: Array<Map<Int, RequirementJisonCell>> = buildList {');
...
for (let chunkIndex = 0; chunkIndex < stateChunks.length; chunkIndex += 1) {
  lines.push(
    `    private fun stateChunk${chunkIndex}(): Array<Map<Int, RequirementJisonCell>> = arrayOf(`,
  );
...
lines.push('    val lexerPatterns: List<RequirementJisonLexerPattern> = buildList {');
...
for (let chunkIndex = 0; chunkIndex < lexerChunks.length; chunkIndex += 1) {
  lines.push(
    `    private fun lexerChunk${chunkIndex}(): List<RequirementJisonLexerPattern> = listOf(`,
  );
  for (
    let ruleIndex = 0;
    ...
    lines.push(
      `        RequirementJisonLexerPattern(${absoluteIndex}, ${kotlinString(rule.source)}),`,
    );
  }
  lines.push('    )');
  lines.push('');
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonMain/kotlin/com/swithun/cmpmermaid/core/requirement/upstream/mermaid/RequirementJisonTables.kt',
);
