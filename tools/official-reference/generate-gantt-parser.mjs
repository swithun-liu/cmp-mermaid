...
const GANTT_JISON_SHA256 =
  '14d325aa626a7fa220c341a75faecc712d0121203e0f88db6c11e68f0bc7e82f';
const GANTT_MARKER = '// src/diagrams/gantt/parser/gantt.jison\n';
const GANTT_END_MARKER = '\nparser.parser = parser;';
...
if (sourceRoot) {
  ...
  const grammarHash = crypto
    ...
    .digest('hex');
  if (grammarHash !== GANTT_JISON_SHA256) {
    throw new Error(
      `Expected Gantt grammar ${GANTT_JISON_SHA256}, found ${grammarHash}`,
    );
  }
}
...
const ganttEntry = fs
  ...
  .find((name) => /^ganttDiagram-.*\.mjs$/.test(name));

if (!ganttEntry) {
  throw new Error('Unable to locate the Mermaid Gantt distribution entry');
}

const ganttChunk = fs.readFileSync(path.join(chunkDirectory, ganttEntry), 'utf8');
const parserStart = ganttChunk.indexOf(GANTT_MARKER) + GANTT_MARKER.length;
if (parserStart < GANTT_MARKER.length) {
  throw new Error('Unable to locate the generated Mermaid Gantt Jison parser');
}
const parserEnd = ganttChunk.indexOf(GANTT_END_MARKER, parserStart);
if (parserEnd < 0) {
  throw new Error('Unable to find the end of the generated Gantt parser');
}

const parserSource = `${ganttChunk.slice(parserStart, parserEnd)}\nparser`;
...
const cellSource = (cell) => {
  if (typeof cell === 'number') {
    return `GanttJisonCell.Goto(${cell})`;
  }
  ...
  switch (cell[0]) {
    case 1:
      return `GanttJisonCell.Shift(${cell[1]})`;
    case 2:
      return `GanttJisonCell.Reduce(${cell[1]})`;
    case 3:
      return 'GanttJisonCell.Accept';
    default:
...
lines.push(` * Upstream gantt.jison SHA-256: ${GANTT_JISON_SHA256}`);
lines.push(
  ` * Distribution chunk SHA-256: ${crypto.createHash('sha256').update(ganttChunk).digest('hex')}`,
);
lines.push(' *');
lines.push(' * Do not edit manually. Run:');
lines.push(' *   cd tools/official-reference && npm run generate:gantt-parser');
lines.push(' */');
lines.push('internal object GanttJisonTables {');
...
lines.push('    )');
lines.push('');
lines.push('    val productions: Array<GanttJisonProduction> = arrayOf(');
for (const production of parser.productions_) {
  if (production === 0) {
    lines.push('        GanttJisonProduction(symbol = 0, length = 0),');
  } else {
    lines.push(
      `        GanttJisonProduction(symbol = ${production[0]}, length = ${production[1]}),`,
    );
  }
}
lines.push('    )');
...
lines.push('    val states: Array<Map<Int, GanttJisonCell>> = buildList {');
...
for (let chunkIndex = 0; chunkIndex < stateChunks.length; chunkIndex += 1) {
  lines.push(
    `    private fun stateChunk${chunkIndex}(): Array<Map<Int, GanttJisonCell>> = arrayOf(`,
  );
...
lines.push('    val lexerPatterns: List<GanttJisonLexerPattern> = buildList {');
...
for (let chunkIndex = 0; chunkIndex < lexerChunks.length; chunkIndex += 1) {
  lines.push(
    `    private fun lexerChunk${chunkIndex}(): List<GanttJisonLexerPattern> = listOf(`,
  );
  for (
    let ruleIndex = 0;
    ...
    lines.push(
      `        GanttJisonLexerPattern(${absoluteIndex}, ${kotlinString(rule.source)}),`,
    );
  }
  lines.push('    )');
  lines.push('');
