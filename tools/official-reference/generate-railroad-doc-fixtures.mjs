...
const packageMetadata = JSON.parse(
  fs.readFileSync(path.join(mermaidSourceRoot, 'packages/mermaid/package.json'), 'utf8'),
);
if (packageMetadata.version !== MERMAID_VERSION) {
  throw new Error(`Expected Mermaid source ${MERMAID_VERSION}, found ${packageMetadata.version}`);
}
...
const documentationHash = crypto.createHash('sha256').update(documentation).digest('hex');
const cases = [];
let heading = 'Railroad';
let fenceLanguage = '';
let fenceLines = null;
...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => (
        /^railroad-(?:ebnf|abnf|peg)-beta(?:\s|$)/.test(candidate) ||
        /^railroad-beta(?:\s|$)/.test(candidate)
      ));
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidRailroadDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/' +
    'OfficialRailroadDocumentationCases.kt',
);
...
console.log(`Generated ${cases.length} Mermaid ${MERMAID_VERSION} Railroad fixtures.`);
