...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => candidate === 'mindmap');
    if (
      ...
      cases.push({
        ...
        title: heading,
        source,
        expectedUnsupportedFeature: source.includes('::icon(')
          ? 'Mindmap icon'
          : source.includes(':::')
            ? 'Mindmap CSS class'
            : null,
...
const kotlinCases = cases
  .map(({ id, title, source, expectedUnsupportedFeature }) => `    MermaidMindmapDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  ...
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/' +
    'OfficialMindmapDocumentationCases.kt',
);
...
console.log(
  `Generated ${cases.length} Mermaid ${MERMAID_VERSION} Mindmap documentation fixtures.`,
);
...
function kotlinRawString(value) {
  if (value.includes('"""')) {
    throw new Error('Mindmap fixture contains a Kotlin raw-string delimiter');
  }
  return `\"\"\"\n${value}\n        \"\"\".trimIndent()`;
}
