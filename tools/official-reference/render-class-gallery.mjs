...
const sourcePath = resolve(
  repositoryRoot,
  'sample/androidApp/src/main/kotlin/io/github/cmpmermaid/sample/ClassDemos.kt',
);
const output = resolve(
  repositoryRoot,
  process.env.OUTPUT_DIR ?? 'captures/local/class-official-puppeteer',
);
...
const layout = process.env.CLASS_LAYOUT ?? 'elk';
...
const casePattern =
  /ClassDemo\(\s*id = "([^"]+)",\s*title = "([^"]+)",\s*category = "([^"]+)",\s*source = """\n([\s\S]*?)\n\s*"""\.trimIndent\(\),\s*\)/g;
...
if (cases.length === 0) {
  throw new Error('No Class gallery cases matched ClassDemos.kt');
}
  ...
  for (const demo of cases) {
    await page.evaluate(
      async ({ id, source, layout }) => {
        ...
        const { svg } = await mermaid.render(`official-class-${id}`, source, container);
...
console.log(`Rendered ${cases.length} Class gallery references to ${output}`);
