...
const sourcePath = resolve(
  repositoryRoot,
  'mermaid-debug-ui/src/commonMain/kotlin/com/swithun/cmpmermaid/debugui/GanttDemos.kt',
);
const output = resolve(
  repositoryRoot,
  process.env.OUTPUT_DIR ?? 'captures/local/gantt-official',
);
...
if (cases.length === 0) {
  throw new Error('No Gantt gallery cases matched GanttDemos.kt');
}
  ...
  for (const demo of cases) {
    await page.evaluate(
      async ({ id, source }) => {
        ...
        const { svg } = await mermaid.render(`official-gantt-${id}`, source, container);
...
console.log(`Rendered ${cases.length} Gantt gallery references to ${output}`);
