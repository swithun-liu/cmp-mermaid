...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^architecture-beta(?:\s|$)/.test(candidate));
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidArchitectureDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialArchitectureDocumentationCases.kt',
);
...
console.log(`Generated ${cases.length} Mermaid ${MERMAID_VERSION} Architecture fixtures.`);
