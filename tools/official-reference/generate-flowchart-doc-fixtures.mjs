...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^(flowchart|graph)\b/.test(candidate));
    if (
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidFlowchartDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialFlowchartDocumentationCases.kt',
);
...
/**
 * Generated from Mermaid ${MERMAID_VERSION} packages/mermaid/src/docs/syntax/flowchart.md.
...
console.log(`Generated ${cases.length} Mermaid ${MERMAID_VERSION} Flowchart documentation fixtures.`);
...
function kotlinRawString(value) {
  if (value.includes('"""')) {
    throw new Error('Flowchart fixture contains a Kotlin raw-string delimiter');
  }
  return `"""
${value.replaceAll('$', "${'$'}")}
        """.trimIndent()`;
