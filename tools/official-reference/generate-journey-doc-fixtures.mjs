...
const documentationHash = crypto
  ...
  .digest('hex');
const cases = [];
let heading = 'User Journey diagrams';
let fenceLanguage = '';
let fenceLines = null;
...
for (const line of documentation.split(/\r?\n/)) {
  ...
  if (line === '```') {
    ...
    const hasDiagramHeader = source
      ...
      .some((candidate) => /^journey\b/.test(candidate));
    if (
...
const kotlinCases = cases
  .map(({ id, title, source }) => `    MermaidJourneyDocCase(
        id = ${kotlinString(id)},
...
const output = path.join(
  repositoryRoot,
  'mermaid-core/src/commonTest/kotlin/com/swithun/cmpmermaid/core/OfficialJourneyDocumentationCases.kt',
);
...
console.log(
  `Generated ${cases.length} Mermaid ${MERMAID_VERSION} User Journey documentation fixtures.`,
);
...
function kotlinRawString(value) {
  if (value.includes('"""')) {
    throw new Error('Journey fixture contains a Kotlin raw-string delimiter');
  }
  return `"""
${value.replaceAll('$', "${'$'}")}
        """.trimIndent()`;
