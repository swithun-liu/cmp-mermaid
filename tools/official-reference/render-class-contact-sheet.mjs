...
const nativeDirectory = resolve(
  repositoryRoot,
  process.env.NATIVE_DIR ?? 'captures/local/class-native-audit-v3',
);
const officialDirectory = resolve(
  repositoryRoot,
  process.env.OFFICIAL_DIR ?? 'captures/local/class-official-all',
);
const outputDirectory = resolve(
  repositoryRoot,
  process.env.OUTPUT_DIR ?? 'captures/local/class-contact-sheets',
);
const casesPerSheet = Number.parseInt(process.env.CASES_PER_SHEET ?? '9', 10);
...
if (caseIds.length === 0) {
  throw new Error('No matching Native/Official Class screenshots were found');
}
...
try {
  for (let start = 0; start < caseIds.length; start += casesPerSheet) {
    ...
    await page.setViewport({
      width: 760,
      height: currentCases.length * 620,
      deviceScaleFactor: 1,
    });
    ...
    await page.screenshot({
      path: resolve(outputDirectory, `class-contact-${sheetNumber}.png`),
      fullPage: true,
...
console.log(
  `Rendered ${caseIds.length} Class comparison pairs to ${outputDirectory}`,
);
