...
class OfficialTreemapDocumentationTest {
    ...
    @Test
    fun rendersEveryOfficialTreemapDocumentationExample() {
        assertEquals(8, officialTreemapDocumentationCases.size)
        officialTreemapDocumentationCases.forEach { fixture ->
            val result = engine.render(fixture.source, context)
            val scene = assertIs<GMResult.Ok<MermaidScene>>(
                result,
                "Expected official Treemap fixture ${fixture.id} to render: $result",
            ).value
