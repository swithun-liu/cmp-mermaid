...
> [!WARNING]
> This report covers the current 24-family implemented subset, not a current
> overall Stable decision. All implemented families have passed replacement
> detail review, but 9 official Mermaid families remain untranslated. See the
> [33-family roadmap](full-diagram-roadmap.md) for the complete status and
> promotion gate.

This report records current evidence for CMP Mermaid's 24 implemented families
...
| Independent production scenarios | 314: 122 release-candidate cases plus 192 additional conformance cases |
| Declared capability coverage | 509/509 points across 24 diagram types |
| Large-scale visual matrix | 6,144 unique Mermaid sources: 256 per diagram type |
| Native core render results | 314 independent plus 6,144 matrix cases passed, 0 failed |
| Web Native/Official captures | 12,288 matrix screenshots plus 628 independent-corpus screenshots, 0 render errors |
| Manual visual review | 384 replacement-gate sheets across all 24 implemented families |
| Automated visual geometry | 6,144/6,144 matrix pairs and 314/314 independent pairs passed |
| Deterministic SceneGraph replay | 314 passed, 0 mismatches |
| Built-in theme matrix | 264/264 renders passed: 24 diagram types by 11 themes |
| Separate deterministic Native stress inputs | 6,144 |
| JVM tests | 619 passed, 0 failed |
| Core production soak | 1,570 renders; 590ms total; 1ms P95; 48,656 bytes retained heap |
...
**Current conclusion:** all 24 implemented families pass the shared core,
...
All 24 implemented families have completed the replacement visual gate. Each
...
6,144/6,144 pairs across 384 manually reviewed sheets:
`5,401 automatic pass / 743 manually reviewed / 0 unresolved`. The 60 ER reviews are
...
Cynefin, and Agentflow each contribute 256 automatic passes. All reviewed cases
...
remaining 9 official families are translated and all 33 family gates pass.
...
1. **314 independent production scenarios.** These are hand-authored,
...
2. **6,144 Native/Official visual-matrix cases.** Each diagram type contributes
...
3. **6,144 Native-only randomized stress inputs.** These separately exercise
...
It includes 122 release-candidate cases plus 192 conformance cases that are
...
Agentflow, and Kanban:

- 6,144 unique Mermaid sources;
- 6,144 CMP Native screenshots;
- 6,144 Mermaid.js `12.0.0` screenshots;
- 384 paged contact sheets, with 16 same-source pairs per page;
- source, seed, profile, feature, screenshot, and SHA-256 metadata;
- per-pair content bounds and foreground-density metrics.

**[Open all 384 paged Native/Official comparison images](assets/stability-report/visual-parity-evidence.md).**
...
The geometry gate accepted all 6,144 pairs. Across the complete matrix,
...
permission audit, 628-image capture, visual geometry gate, and Web load test
on every push to `main` and every pull request. The
[`Full Visual Parity`](../.github/workflows/full-visual-parity.yml) workflow
runs the 6,144-pair matrix weekly and on demand in 24 parallel diagram jobs.
...
It compiles all 314 sources and rejects parser errors, invalid or non-finite
geometry, scenes outside the 20,000-unit and 20,000-element limits, empty
SceneGraphs, and non-deterministic replay. The 192 conformance cases also
require selected semantic text to survive parsing and layout. The test renders
a representative of all 24 implemented diagram types with each of the 11 built-in
themes.

`ProductionCorpusTest` additionally renders all 6,144 visual-matrix sources,
...
Native-only inputs for each of the 24 implemented diagram types, for 6,144
...
Every one of the 314 independent scenarios is rendered twice and compared as a
...
rounds over all 314 scenarios:

```text
renders=1570
totalMs=590
p95Ms=1
retainedHeapBytes=48656
```
...
| Web | 275 | Prior baseline passed | 0.63s first content; 12.54s scroll; 10.6MiB retained JS heap; final Ishikawa case reached; Cynefin not included |
...
Set `AUDIT_KIND` and `CORPUS_KIND` to one of the 24 diagram IDs to reproduce
...
These criteria are not currently satisfied. The current 24 implemented
families have completed the semantic, paint-order, and perceptual re-audit,
but 9 official Mermaid families are not yet implemented. The public code
