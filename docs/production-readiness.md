...
- Implemented families: 21; pending families: 12
...
- Completed replacement detail gates: Flowchart, XY Chart, Quadrant Chart,
  ...
  Treemap, Venn, and Ishikawa (`5,376/5,376` accepted; `4,889 automatic pass / 487 manually reviewed / 0 unresolved`;
...
| --- | --- | --- | --- |
| Visual fidelity | No known semantic or major visual mismatch in every family corpus | All 21 implemented families have 5,376/5,376 accepted detail and geometry pairs: 4,889 automatic passes plus 487 manually accepted ER/Journey/Requirement/Git Graph/Mindmap/Treemap/Venn reviews | **Implemented subset passing; 12 families remain untranslated** |
| Capability coverage | Every Mermaid 12.0.0 family and declared major capability has an independent conformance case | 424/424 points across 21 implemented families; 12 families remain | **Incomplete** |
| Determinism | Repeated rendering returns the same SceneGraph | Full 275-case corpus equality test for the implemented subset | Passing for the implemented subset |
| Theme compatibility | Every implemented diagram family renders with every applicable built-in theme | 21 by 11 matrix: 231/231 renders | Passing for the implemented subset |
| Parser/layout robustness | Systematic matrix and deterministic randomized corpus pass resource limits | 5,376 visual-matrix Native renders plus a separate 5,376 generated stress inputs | Passing for the implemented subset |
| Core throughput | 1,375 warmed production renders complete within 45s and P95 is at most 500ms | Local baseline: 436ms total, 1ms P95 | Passing; enforced by JVM test |
| Core retained heap | The same soak retains at most 64 MiB after forced GC | Local baseline: 45,320 bytes | Passing; enforced by JVM test |
| Runtime matrix | Android, iOS Simulator, Desktop, and Web render representative complex cases | Web reached the final Ishikawa case across all 275 current scenarios; Android, iOS, and Desktop retain the prior 236-scenario baseline | Current Web run passing; prior native baselines passing |
| Runtime load | A scrolling page with many mixed diagrams stays responsive and within a documented memory budget | Current 275-diagram Web run and prior 236-diagram native runs recorded below | Passing for the recorded corpus on each platform |
...
The regular Quality Gate runs the 275 independent cases on every push and pull
request. The weekly/manual Full Visual Parity workflow adds 256 unique sources
per diagram type, or 5,376 Native/Official pairs total. Those matrix cases are
...
| Web | 275 | 0.63s first content; 12.54s scroll | 10.6MiB retained JS heap | Passed 15s/96MiB budget through final Ishikawa case |
...
The full rule is not currently satisfied. All 21 implemented families have
