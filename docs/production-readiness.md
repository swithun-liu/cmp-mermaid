...
- Native renderers: Flowchart, XY Chart, Sequence, Class, State, Entity
  Relationship, Gantt, Pie, User Journey, Requirement, and Git Graph
...
| --- | --- | --- | --- |
| Visual fidelity | No known semantic or major visual mismatch in the independent production corpus | 145 manually reviewed Native/Official pairs plus 2,816 systematic matrix pairs; all 2,961 pass automated geometry | Passing locally; regular and full-matrix workflows are source controlled |
| Capability coverage | Every declared major capability appears in an independent conformance case | 186/186 points across 11 diagram types | Passing; generated-corpus validation enforces coverage |
| Determinism | Repeated rendering returns the same SceneGraph | Full 145-case corpus equality test | Passing |
| Theme compatibility | Every supported diagram type renders with every built-in theme | 11 diagram types by 11 themes; 121/121 renders | Passing |
| Parser/layout robustness | Systematic matrix and deterministic randomized corpus pass resource limits | 2,816 visual-matrix Native renders plus a separate 2,816 generated stress inputs | Passing |
| Core throughput | 725 warmed production renders complete within 45s and P95 is at most 500ms | Local baseline: 8.89s total, 61ms P95 | Passing; enforced by JVM test |
| Core retained heap | The same soak retains at most 64 MiB after forced GC | Local baseline: about 23 KiB | Passing; enforced by JVM test |
...
| Runtime load | A scrolling page with many mixed diagrams stays responsive and within a documented memory budget | 145-diagram matrix recorded below | Passing on Android, iOS, Desktop, and Web |
...
The regular Quality Gate runs the 145 independent cases on every push and pull
request. The weekly/manual Full Visual Parity workflow adds 256 unique sources
per diagram type, or 2,816 Native/Official pairs total. Those matrix cases are
...
| --- | ---: | --- | --- | --- |
| Android Emulator | 145 | 30s list traversal | 237MiB peak PSS; 208MiB final PSS | Passed 300MiB/45s budget |
| iOS Simulator | 145 | Automatic traversal reached final case | About 518MiB final host RSS | Passed without a crash |
| Desktop | 145 | Process and window survived the 10.88s automatic traversal | About 91MiB process RSS | Passed without a crash |
| Web | 145 | 0.68s first content; 5.05s scroll | 9.2MiB retained JS heap | Passed 15s/96MiB budget |
