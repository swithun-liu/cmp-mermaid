...
- Native renderers: Flowchart, XY Chart, Quadrant Chart, Timeline, Kanban,
  ...
  Requirement, Git Graph, Mindmap, Packet, Radar, and Sankey
...
- Implemented families: 18; pending families: 15
...
- Completed replacement detail gates: Flowchart, XY Chart, Quadrant Chart,
  ...
  User Journey, Requirement, Git Graph, Mindmap, Packet, Radar, and Sankey (`4,608/4,608`
  accepted; `4,398 pass / 210 manually reviewed / 0 fail`; every implemented-family
...
| --- | --- | --- | --- |
| Visual fidelity | No known semantic or major visual mismatch in every family corpus | All 18 implemented families have 4,608/4,608 accepted detail and geometry pairs: 4,398 automatic passes plus 210 manually accepted ER/Journey/Requirement/Git Graph/Mindmap reviews | **Implemented subset passing; 15 families remain untranslated** |
| Capability coverage | Every Mermaid 12.0.0 family and declared major capability has an independent conformance case | 326/326 points across 18 implemented families; 15 families remain | **Incomplete** |
| Determinism | Repeated rendering returns the same SceneGraph | Full 236-case corpus equality test for the implemented subset | Passing for the implemented subset |
| Theme compatibility | Every implemented diagram family renders with every applicable built-in theme | 18 by 11 matrix: 198/198 renders | Passing for the implemented subset |
| Parser/layout robustness | Systematic matrix and deterministic randomized corpus pass resource limits | 4,608 visual-matrix Native renders plus a separate 4,608 generated stress inputs | Passing for the implemented subset |
| Core throughput | 1,180 warmed production renders complete within 45s and P95 is at most 500ms | Local baseline: 446ms total, 1ms P95 | Passing; enforced by JVM test |
| Core retained heap | The same soak retains at most 64 MiB after forced GC | Local baseline: 35,144 bytes | Passing; enforced by JVM test |
...
| Runtime load | A scrolling page with many mixed diagrams stays responsive and within a documented memory budget | 236-diagram matrix recorded below | Passing on Android, iOS, Desktop, and Web |
...
The regular Quality Gate runs the 236 independent cases on every push and pull
request. The weekly/manual Full Visual Parity workflow adds 256 unique sources
per diagram type, or 4,608 Native/Official pairs total. Those matrix cases are
...
| --- | ---: | --- | --- | --- |
| Android Emulator | 236 | See current machine-readable evidence | See current machine-readable evidence | Passed current budget |
| iOS Simulator | 236 | See current machine-readable evidence | See current machine-readable evidence | Passed without a crash |
| Desktop | 236 | See current machine-readable evidence | See current machine-readable evidence | Passed without a crash |
| Web | 236 | See current machine-readable evidence | See current machine-readable evidence | Passed 15s/96MiB budget |
...
The full rule is not currently satisfied. All 18 implemented families have
